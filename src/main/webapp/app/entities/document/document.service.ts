...
export class DocumentService {
  ...
  protected convertDateFromClient(document: IDocument): IDocument {
    const copy: IDocument = Object.assign({}, document, {
      defenseDate: document.defenseDate && document.defenseDate.isValid() ? document.defenseDate.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.defenseDate = res.body.defenseDate ? moment(res.body.defenseDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((document: IDocument) => {
        document.defenseDate = document.defenseDate ? moment(document.defenseDate) : undefined;
      });
    }
    return res;
  }
